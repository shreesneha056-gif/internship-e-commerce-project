...
with tab1:
    ...
    with col1:
        ...
        utm_content = st.selectbox("Ad content (utm_content)", ["none"])  # extend if you used more values in training
    ...
    with col2:
        ...
        landing_page = st.selectbox("Landing page", ["/home", "/lander-1", "/lander-2", "/lander-3", "/lander-4", "/lander-5", "other"])

    if st.button("Predict conversion", key="btn_conversion"):
        model = joblib.load("conversion_model_final_rf.pkl")
        train_columns = joblib.load("conversion_model_columns.pkl")
...
with tab2:
    ...
    if st.button("Predict refund risk", key="btn_refund"):
        bundle = joblib.load("refund_model.pkl")
        model = bundle["model"]
...
with tab3:
    ...
    if st.button("Forecast revenue", key="btn_forecast"):
        sarimax_model = joblib.load("sarimax_revenue_forecast_model.pkl")
