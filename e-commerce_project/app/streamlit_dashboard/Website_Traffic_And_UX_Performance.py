...
# KPI row
# ------------------------------------------------------------------
all_devices = list(d.sessions_by_device["device_type"])
is_full_selection = not device_filter or set(device_filter) == set(all_devices)
...
if is_full_selection:
    # Matches the report's exact published total (473K) rather than the sum of
    # the rounded 0.33M / 0.15M bars, which would read 480K.
    total_sessions_display = d.KPI_TRAFFIC["Total_Sessions"]
else:
    total_sessions_display = f"{device_df['Total_Sessions'].sum()/1000:.0f}K"
